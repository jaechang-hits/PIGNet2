...
def run(protein_id, lines):
    ...
    id_to_pred = {}
    for l in lines:
        complex_id = "_".join(l[0].split("_")[:-1])
        if complex_id not in id_to_pred:
            id_to_pred[complex_id] = []
        id_to_pred[complex_id].append(float(l[2]))
    id_to_pred = {i: min(id_to_pred[i]) for i in id_to_pred.keys()}
